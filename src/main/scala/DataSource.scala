...
class DataSource(ep: EmptyParams)
  ...
  override def readEval(sc: SparkContext): Seq[(TrainingData, EmptyEvaluationInfo, RDD[(Query, ActualResult)])] = {
    ...
    val rdd = events.map {
      case (_, properties) =>
        Query(properties.get[Array[String,Double]]("vector")) -> ActualResult(properties.get[Double]("label"))
    }
  ...
  def selectEvents(sc: SparkContext): RDD[(String, PropertyMap)] = {
    ...
    val events = grades.map {
      case (entityId, properties) =>
        val fields = Map(
          "vector" -> JArray(List(
             JString(properties.get[String]("vendor")),JString(properties.get[String]("item")),JDouble(properties.get[Double]("month")),JDouble(properties.get[Double]("day"))
           )),
